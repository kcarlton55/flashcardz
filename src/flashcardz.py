...
def go(shuffle=True):
    ...
    '''
    date_format = _settings['date_format']
    _cards = _open()
    ...
    if shuffle == True:
        ...
        print()
        try:
            _cards = sorted(_cards, key=lambda x: random.random())
        except:
            pass
    ...
    for i, _card in enumerate(_cards):
        _card[3] = int(_card[3]) + 1   # card[3] is "viewed"
        number = i + 1
        loop = True
        flag = True
        hide_url = True
        while loop:
            print(25*'-' + ' tally: ' + str(_card[4]) + ' ' + 25*'-')
            print(f'{number} of {number_of_cards}.  {_card[0]}')  # card[0] is "word"
            if flag:  # pause after word shown, but pause only once.
                ...
                if ans0 and (ans0[0].lower() == 'q' or ans0[0].lower() == 'e'):
                    return
            if hide_url:
                desc = _hide_urls(_card[1])  # card[1] is "description"
            else:
                desc = _card[1]
            #text = f'{desc}'
            print(f'{desc}\n')
            ...
            if ans and ans.isnumeric():
                hide_url = True
                webbrowser.open(_url_at(_card[1], int(ans)))
                ...
                print()
                _card[4] = max(0,  _settings['maxtally'] - _settings['tallypenalty'])
                missed.append(_card)
                loop = False
            elif ans and ans[0].lower() == 'y':
                print()
                _card[4] = int(_card[4]) + 1    # card[4] is "tally"
                if _card[4] >= _settings['maxtally']:
                    unwanted.append(i)
                loop = False
            else:
                print()
                _card[4] = int(_card[4]) + 1
                if _card[4] >= _settings['maxtally']:
                    unwanted.append(i)
                loop = False
    print("\n             === The End ===")
